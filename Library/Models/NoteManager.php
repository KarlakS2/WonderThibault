...
class NoteManager extends \Library\Manager{
	...
	public function add(Entity $note){
		//Securite
		$number = $note->getNote();
		$phrase_id = $note->getPhrase_id();
		$user_id = $note->getUser_id();

		$req = $this->_db->prepare('INSERT INTO note(id,note,user_id,phrase_id) VALUES (NULL, :note, :user_id, :phrase_id)');
	...
	public function get($phrase_id,$user_id){
		$req = $this->_db->prepare('SELECT * FROM note WHERE phrase_id = :phrase_id AND user_id = :user_id');
    	$req->bindParam(':phrase_id', $phrase_id,\PDO::PARAM_INT);
    	$req->bindParam(':user_id', $user_id,\PDO::PARAM_INT);
    	$req->execute();
	...
	public function average($id){
		$req = $this->_db->prepare('SELECT ROUND(AVG(note)) AS AVG FROM note WHERE phrase_id = :id');
    	$req->bindParam(':id', $id,\PDO::PARAM_INT);
    	$req->execute();
