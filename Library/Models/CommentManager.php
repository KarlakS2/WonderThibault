...
class CommentManager extends \Library\Manager{
	...
	public function add(Entity $comment){
		...
		//Securite
		$phrase_id = $comment->getPhrase_id();
		$user_id = $comment->getUser_id();
		$text = $comment->getComment();
		$date = $comment->getDate_publish();

		$req = $this->_db->prepare('INSERT INTO comment(id,phrase_id,user_id,comment,date_publish) VALUES (NULL, :phrase_id, :user_id,:comment,:date_publish)');
		$req->bindParam(':phrase_id',$phrase_id,\PDO::PARAM_INT);
	...
	public function getAll($phrase_id){
		$comments = array();

		$req = $this->_db->prepare('SELECT c.comment,  DATE_FORMAT(c.date_publish, \'Le %d/%m/%Y à %Hh%i\') AS date_fr, c.date_update, u.name, u.avatar FROM comment c INNER JOIN user u ON u.id = c.user_id WHERE c.phrase_id = :phrase_id ORDER BY date_publish');
		$req->bindParam(':phrase_id', $phrase_id,\PDO::PARAM_INT);
		$req->execute();
